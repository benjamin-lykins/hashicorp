...
resource "aws_security_group_rule" "lb_allow_ingress_nomad_from_cidr" {
  ...
  description = "Allow TCP/443 (HTTPS) inbound to Nomad load balancer from specified CIDR ranges."
...
resource "aws_security_group_rule" "lb_allow_ingress_nomad_from_ec2" {
  ...
  description              = "Allow TCP/443 (HTTPS) inbound to Nomad load balancer from Nomad EC2 security group."
